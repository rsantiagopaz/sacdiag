...
class class_Parametros extends class_Base
{
  ...
  public function method_autocompletarPersona($params, $error) {
	...
	if (is_numeric($p->texto)) {
		$sql = "SELECT persona_id AS model, CONCAT(persona_dni, ' - ', persona_nombre) AS label, persona_tipodoc, persona_dni FROM _personas WHERE persona_dni LIKE '". $p->texto . "%' ORDER BY label";
	} else {
		$sql = "SELECT persona_id AS model, CONCAT(TRIM(persona_nombre), ' (', persona_dni, ')') AS label, persona_tipodoc, persona_dni FROM _personas WHERE persona_nombre LIKE '%". $p->texto . "%' ORDER BY label";
	}
  ...
  public function method_autocompletarEstablecimiento($params, $error) {
  	$p = $params[0];
  	
  	$sql = "SELECT id_ta_establecimiento AS model, descrip AS label FROM ta_establecimiento WHERE descrip LIKE '%" . $p->texto . "%' ORDER BY label";
	return $this->toJson($sql);
  }
  
  
  public function method_autocompletarCie10($params, $error) {
  	$p = $params[0];
  	
  	$sql = "SELECT id_diagnostico AS model, CONCAT(descripcion4, ' (', cod_4, ')') AS label FROM cie10 WHERE descripcion4 LIKE '%" . $p->texto . "%' ORDER BY label";
	return $this->toJson($sql);
  }
