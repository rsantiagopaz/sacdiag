...
class class_M_Solicitudes extends class_Base
{
  ...
  public function method_leer_solicitud($params, $error) {
	...
	$sql = "SELECT m_solicitudes.*, _personas.persona_nombre, _personas.persona_dni FROM m_solicitudes INNER JOIN _personas USING(persona_id) WHERE TRUE";
  ...
  public function method_leer_solicitudes_prestaciones($params, $error) {
	...
	$sql.= " FROM m_solicitudes_items INNER JOIN m_vademecum USING(id_m_vademecum) INNER JOIN m_tipo_producto USING(id_m_tipo_producto) INNER JOIN m_unidades USING(id_m_unidad)";
	$sql.= " WHERE id_m_solicitud=" . $p->id_m_solicitud;
