...
class class_M_Solicitudes extends class_Base
{
  ...
  public function method_entregar_item($params, $error) {
	$p = $params[0];
	
	$this->mysqli->query("START TRANSACTION");
	
	$cantidad_entregada = $p->cantidad_entregada + $p->cantidad;
	$estado = ($cantidad_entregada < $p->cantidad_pedida) ? "EI" : "EC";
	$sql = "UPDATE m_solicitudes_items SET cantidad_entregada = " . $cantidad_entregada . ", estado='" . $estado . "' WHERE id_m_solicitud_item=" . $p->id_m_solicitud_item;
	$this->mysqli->query($sql);
