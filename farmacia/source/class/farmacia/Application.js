...
/**
 * This is the main application class of your custom application "farmacia"
 *
 * @asset(farmacia/*)
 */
qx.Class.define("farmacia.Application",
{
  ...
  members :
  {
	...
	if (id_version == resultado.id_version) {
		qx.event.Timer.once(function(){
			var win = new farmacia.comp.windowLogin();
			win.setModal(true);
	...
	var loading = this.loading = new componente.comp.ui.ramon.image.Loading("farmacia/loading66.gif");
	
	farmacia.comp.rpc.Rpc.LOADING = loading;
	...
	btnAcercaDe.addListener("execute", function(){
		var win = new farmacia.comp.windowAcercaDe();
		win.setModal(true);
		doc.add(win);
		win.center();
		win.open();
	...
	btnMedicamentos.addListener("execute", function(){
		if (pageMain["pageMedicamentos"] == null) {
			pageMain["pageMedicamentos"] = new farmacia.comp.pageMedicamentos();
	...
	btnEstadisticas.addListener("execute", function(){
		if (pageMain["pageParametros"] == null) {
			pageMain["pageParametros"] = new farmacia.comp.pageParametros();
	...
	btnWebServices.addListener("execute", function(){
		var win = new farmacia.comp.windowWebService("");
		win.setModal(true);
		doc.add(win);
		win.center();
		win.open();
	...
	btnContrasena.addListener("execute", function(e){
		var win = new farmacia.comp.windowContrasena();
		win.setModal(true);
		doc.add(win);
		win.center();
		win.open();
